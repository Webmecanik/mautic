...
namespace MauticPlugin\MauticYellowboxCrmBundle\Service;
...
class LeadEventSupplier
{
    ...
    public function getMappedLeadIds(): array
    {
        ...
        $statement = $connection->prepare('select group_concat(l.id) as ids from '.MAUTIC_TABLE_PREFIX.'sync_object_mapping map
          ...
          where map.integration = 'YellowboxCrm' and map.internal_object_name = 'lead' and map.is_deleted = 0");
        ...
        foreach ($leadIds as $leadId) {
            ...
            while (true) {
                ...
                foreach ($events as $event) {
                    ...
                    $yellowboxCheck[$leadId][$event['timestamp']->getTimestamp()][] = $checkEvent;
                }
                ++$page;
