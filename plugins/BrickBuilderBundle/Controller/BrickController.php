...
class BrickController extends CommonController
{
    ...
    public function builderAction($objectType, $objectId)
    {
        ...
        /** @var MjmlHelper $mjmlHelper */
        $mjmlHelper = $this->get('brickbuilder.helper.mjml');

        // Check for MJML template
        if ($logicalName = $mjmlHelper->checkForMjmlTemplate($templateName.'.mjml.twig')) {
            $type        = 'mjml';
        } else {
        ...
        return $this->render(
            'BrickBuilderBundle:Builder:template.html.php',
            [
