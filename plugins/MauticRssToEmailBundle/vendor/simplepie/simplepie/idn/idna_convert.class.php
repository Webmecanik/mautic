    ...
    public function decode($input, $one_time_encoding = false)
    {
        ...
        if (strpos($input, '@')) { // Maybe it is an email address
            ...
            if (isset($parsed['host'])) {
                ...
                $return         =
                        (empty($parsed['scheme']) ? '' : $parsed['scheme'].('mailto' == strtolower($parsed['scheme']) ? ':' : '://'))
    ...
    public function _utf8_to_ucs4($input)
    {
        ...
        for ($k = 0; $k < $inp_len; ++$k) {
            ...
            if ('next' == $mode) { // Try to find the next start byte; determine the width of the Unicode char
                ...
                $mode       = 'add';
                $test       = 'range';
                if (6 == $v >> 5) { // &110xxxxx 10xxxxx
                    $next_byte = 0; // Tells, how many times subsequent bitmasks must rotate 6bits to the left
                    $v         = ($v - 192) << 6;
                } elseif (14 == $v >> 4) { // &1110xxxx 10xxxxxx 10xxxxxx
                    $next_byte = 1;
                    $v         = ($v - 224) << 12;
                } elseif (30 == $v >> 3) { // &11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
                    $next_byte = 2;
                    $v         = ($v - 240) << 18;
                } elseif (62 == $v >> 2) { // &111110xx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
                    $next_byte = 3;
                    $v         = ($v - 248) << 24;
                } elseif (126 == $v >> 1) { // &1111110x 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
                    $next_byte = 4;
                    $v         = ($v - 252) << 30;
                } else {
            ...
            if ('add' == $mode) {
                ...
                if (2 == $v >> 6) { // Bit mask must be 10xxxxxx
                    $v = ($v - 128) << ($next_byte * 6);
                    $output[($out_len - 1)] += $v;
                    --$next_byte;
                } else {
